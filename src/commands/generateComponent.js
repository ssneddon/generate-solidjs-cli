...
export default function initGenerateComponentCommand(args, cliConfigFile, program) {
  ...
  const componentCommand = program
    ...
    .option(
      '--type <type>',
      'You can pass a component type that you have configured in your GSC config file.',
      'default'
    )
    .option(
      '-d, --describe <describe>',
      'Describe the component you want GSC to generate (e.g., Create a counter component that increments by one when I click on the increment button).',
      null
    )
    .option(
      '-f, --flat',
